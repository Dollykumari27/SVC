function [x_mmp_s x_supp idx_depth] = algo_mmp(y, Phi, K, L, N)
	...
	while (idx_depth < K-1)
		...
		if N_max < idx_abs_child
			nodes_parents(resi_idx(N_max:end))	= [];
		end
	end
end

%%
function [x_hat reuse_inv] = algo_mmp(y, prev_cols, new_col, reuse_Minv)
